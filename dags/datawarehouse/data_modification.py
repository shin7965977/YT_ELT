...
def insert_rows(cur, conn, schema, row):
    ...
    try:
        if schema == 'staging':
            video_id = 'video_id'
            current_id = row.get(video_id, "Unknown") 
        ...
        conn.commit()
        logger.info(f"Successfully inserted row with Video_ID: {current_id}")

    except Exception as e:
        logger.error(f"Error inserting row with Video_ID: {current_id}. Database error: {e}")
        raise e
...
def delete_rows(cur, conn, schema, ids_to_delete):
    ...
    try:
        sql_ids = f"""({', '.join(f"'{id}'" for id in ids_to_delete)})"""
        
        cur.execute(
            f"""
            DELETE FROM {schema}.{table}
            WHERE "Video_ID" IN {sql_ids};
            """
        )
        
        conn.commit()
        logger.info(f"Successfully deleted rows with Video_IDs: {sql_ids}")
